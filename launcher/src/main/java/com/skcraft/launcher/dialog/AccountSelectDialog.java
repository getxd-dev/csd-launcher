...
public class AccountSelectDialog extends JDialog {
	...
	private void initComponents() {
		...
		offlineButton.addActionListener(ev -> {
			String defaultName = launcher.getProperties().getProperty("offlinePlayerName", "Player");
			String username = (String) JOptionPane.showInputDialog(
					this,
					SharedLocale.tr("login.offlineUsernameRequest"),
					SharedLocale.tr("login.playOffline"),
			...
			if (username != null && !username.trim().isEmpty()) {
				...
				// Basic validation to prevent command line argument injection issues
				if (!username.matches("^[a-zA-Z0-9_]+$")) {
					SwingHelper.showErrorDialog(this,
							SharedLocale.tr("login.offlineUsernameInvalid"),
	...
	public static Session showAccountRequest(Window owner, Launcher launcher) {
		...
		if (dialog.selected != null && (dialog.selected.isOnline() || dialog.selected instanceof OfflineSession)) {
	...
	private void attemptExistingLogin(SavedSession session) {
		...
		if (session.getType() == UserType.LEGACY && "0".equals(session.getAccessToken())) {
